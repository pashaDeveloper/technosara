...
import { useGetInsuranceCompaniesQuery } from "@/services/insuranceCompany/insuranceCompanyApi";
import { useMemo } from "react";
const TitleStep = ({ register, errors, control, prevStep, nextStep }) => {
  const {
    isLoading: fetchingInsuranceCompanies,
    data: fetchInsuranceCompaniesData,
    error: fetchInsuranceCompaniesError,
  ...
  const insuranceCompanies = useMemo(
    () =>
      fetchInsuranceCompaniesData?.data?.map((insuranceCompanies) => ({
        id: insuranceCompanies._id,
        value: insuranceCompanies.name_fa,
        label: insuranceCompanies.name_fa,
        description: insuranceCompanies.description
      })) || [],
