...
} from "@/services/warrantyCompany/warrantyCompanyApi";
...
const WarrantyCompanies = () => {
  ...
  const { data, isLoading, error } = useGetWarrantyCompaniesQuery({
    page: currentPage,
    limit: itemsPerPage,
  ...
  const warrantyCompanies = useMemo(() => data?.data || [], [data]);
  const totalPages = data ? Math.ceil(data.total / itemsPerPage) : 1;

  const [
    deleteWarrantyCompany,
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال دریافت شرکت‌های بیمه...", { id: "warrantyCompaniesData" });
    }

    if (data && data.acknowledgement) {
      toast.success(data.description, { id: "warrantyCompaniesData" });
    }

    if (error) {
      toast.error(error?.data?.description || "خطایی رخ داده است", { id: "warrantyCompaniesData" });
    }
  ...
  useEffect(() => {
    if (isRemoving) {
      toast.loading("در حال حذف شرکت بیمه...", { id: "deleteWarrantyCompany" });
    } else {
      toast.dismiss("deleteWarrantyCompany");
    }

    if (deletedData) {
      toast.success(deletedData.description || "شرکت بیمه با موفقیت حذف شد.", { id: "deleteWarrantyCompany" });
    }

    if (deleteError) {
      toast.error(deleteError?.data?.description || "خطا در حذف شرکت بیمه", { id: "deleteWarrantyCompany" });
    }
  ...
  return (
    <ControlPanel>
      ...
      ) : warrantyCompanies.length === 0 ? (
        <SkeletonItem repeat={5} />
      ) : (
        <section className="w-full h-full">
          <div className="overflow-x-auto w-full">
            {warrantyCompanies.map((company) => (
              <div
                key={company._id}
                ...
                <div className="col-span-10 lg:col-span-4 text-center flex items-center">
                  ...
                  <div className="py-2 flex justify-center items-center gap-x-2 text-right">
                    <img
                      ...
                      alt={company.name_fa || "warranty company image"}
                      height={60}
                      width={60}
                ...
                <div className="col-span-2 md:col-span-1 gap-2 text-center flex justify-center items-center">
                  <article className="lg:flex-row flex flex-col justify-center gap-x-2 gap-y-2">
                    <DeleteModal
                      ...
                      onDelete={() => deleteWarrantyCompany(company._id)}
                    />
                  </article>
                </div>
              </div>
...
export default WarrantyCompanies;
