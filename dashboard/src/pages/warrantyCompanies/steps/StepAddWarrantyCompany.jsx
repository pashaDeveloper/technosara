...
import { useAddWarrantyCompanyMutation } from "@/services/warrantyCompany/warrantyCompanyApi";
...
const StepAddWarranty = () => {
  const [thumbnail, setThumbnail] = useState(null);
  const [addWarranty, { isLoading, data, error }] = useAddWarrantyCompanyMutation();
  ...
  useEffect(() => {
    if (isLoading) {
      toast.loading("در حال افزودن گارانتی...", { id: "addWarranty" });
    }

    if (data &&data?.acknowledgement) {
      toast.success(data?.description, { id: "addWarranty" });
      navigate("/warrantyCompanies");
    }
    if (data&&!data?.acknowledgement) {
      toast.error(data?.description, { id: "addWarranty" });
    }
    if (error?.data) {
      toast.error(error?.data?.description, { id: "addWarranty" });
    }
  ...
  const nextStep = async () => {
    ...
    switch (currentStep) {
      ...
      case 2:
        ...
        if (!valid) {
          toast.error("لطفاً عنوان گارانتی را وارد کنید");
          setInvalidSteps((prev) => ({ ...prev, [currentStep]: true }));
          return;
        }
        valid = await trigger("description");
        if (!valid) {
          toast.error("لطفاً توضیحات گارانتی را وارد کنید");
          setInvalidSteps((prev) => ({ ...prev, [currentStep]: true }));
          return;
        }
        break;
      case 3:
        ...
        if (!valid) {
          toast.error("لطفاً ویژگی های گارانتی را وارد کنید");
          setInvalidSteps((prev) => ({ ...prev, [currentStep]: true }));
          return;
        }
        break;
      case 4:
        ...
        if (!valid) {
          toast.error("لطفاً اطلاعات تماس گارانتی را وارد کنید");
          setInvalidSteps((prev) => ({ ...prev, [currentStep]: true }));
          return;
        }
        break;
      case 5:
        ...
        if (!valid) {
          toast.error("لطفاً ادرس  گارانتی را وارد کنید");
          setInvalidSteps((prev) => ({ ...prev, [currentStep]: true }));
          return;
        }
        break;
...
export default StepAddWarranty;
